...
make_get_mparam(p9320_mfc, lpcharge, unsigned int);
#if defined(CONFIG_WIRELESS_IC_PARAM)
make_get_mparam(p9320_mfc, wireless_ic, unsigned int);
#endif
...
static void mfc_tx_prevent_reconnection(struct mfc_charger_data *charger)
{
#if defined(CONFIG_TX_GEAR_PHM_VOUT_CTRL) || defined(CONFIG_TX_GEAR_AOV)
	u8 data = 0;
...
static void mfc_tx_phm_work(struct work_struct *work)
{
	...
	if (value.intval == TX_SWITCH_CHG_ONLY) {
		...
		mfc_tx_prevent_reconnection(charger);
#endif
#if defined(CONFIG_TX_GEAR_AOV)
	}
#endif
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_EXT_PROP_MIN ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_WIRELESS_AUTH_ADT_DATA: /* data from auth service will be sent */
			if (charger->pdata->cable_type != SEC_BATTERY_CABLE_NONE && !get_mparam(p9320_mfc, lpcharge)) {
				u8 *p_data;
				int i;
	...
	if (cmd_data == WPC_TX_COM_AFC_SET) {
		...
		if (!charger->tx_id_done) {
			...
			case TX_ID_AUTH_PAD ... TX_ID_AUTH_PAD_END:
				...
				if (!get_mparam(p9320_mfc, lpcharge)) {
...
{
	unsigned int param_value = get_mparam(p9320_mfc, wireless_ic);
...
/*#if !defined(CONFIG_SEC_FACTORY)
	else if (!get_mparam(p9320_mfc, lpcharge)) {
